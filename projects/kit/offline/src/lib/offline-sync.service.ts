...
/** Stable lookup retained by a caller while a generated command may be reconciled and removed. */
export interface OfflineGeneratedCommandLocator {
...
/** A foreground durable-command send reached a terminal server-side state. */
export class OfflineImmediateCommandRejectedError extends Error {
  constructor(
    readonly state: 'blocked_auth' | 'rejected' | 'conflict',
    readonly code: string | null,
  ) {
    super(`Immediate offline command delivery ended in ${state}${code ? ` (${code})` : ''}.`);
    this.name = 'OfflineImmediateCommandRejectedError';
  }
}
...
export class OfflineSyncService {
  ...
  readonly #commandSendTransitions = new Map<string, Promise<boolean>>();
  ...
  #assertDistinctBatchFootprints(
    ...
    const commandIds = new Set(existingCommands.map((command) => command.commandId));
    ...
    for (const entry of entries) {
      if (commandIds.has(entry.command.commandId)) {
        throw new Error(`Offline command id ${entry.command.commandId} is already in use.`);
      }
  ...
  /**
   * Sends one newly-created generated aggregate immediately after its durable
   * enqueue, without waiting for a pre-send pull.
   *
   * This fast path is deliberately narrow: the command must still be pending,
   * have no remote id or confirmed baseline, and be the first intent for its
   * aggregate. Existing aggregates continue through {@link flush}, where the
   * pre-pull conflict barrier remains mandatory. Authoritative pull
   * reconciliation is scheduled in the background after the local transport
   * acknowledgement has been committed.
   *
   * @returns The generated remote id when transport completed locally, or
   * `null` when transport is unavailable or did not produce a confirmed id.
   */
  async sendGeneratedCommandNow(commandId: string, locator: OfflineGeneratedCommandLocator): Promise<OfflineGeneratedRemoteId | null> {
    const generation = this.#generation;
    if (!(await this.#restoreCurrentGeneration(generation))) return null;
    const userId = this.#activeUserId;
    if (userId === null) return null;
    return this.#beginGeneratedCommandSend(commandId, locator, generation, userId);
  }
  ...
  #beginGeneratedCommandSend(
    commandId: string,
    locator: OfflineGeneratedCommandLocator,
    generation: number,
    userId: OfflinePrincipalId,
  ): Promise<OfflineGeneratedRemoteId | null> {
    const result = this.#runGeneratedCommandSend(commandId, locator, generation, userId);
    let requiresReconciliation = false;
    const transition = result
      .then(
        (remoteId) => {
          requiresReconciliation = remoteId !== null;
        },
        () => undefined,
      )
      .finally(() => {
        this.#flushTransitions.delete(transition);
        if (requiresReconciliation && this.#canSynchronize() && this.#isCurrentUser(generation, userId)) {
          // A normal flush may already be in its post-pull phase. Ensure the
          // newly acknowledged command receives a later reconciliation pass.
          if (this.#flushPromise) this.#resumeAfterFlush = true;
          queueMicrotask(() => this.#flushInBackground());
        }
      });
    this.#flushTransitions.add(transition);
    return result;
  }

  async #runGeneratedCommandSend(
    commandId: string,
    locator: OfflineGeneratedCommandLocator,
    generation: number,
    userId: OfflinePrincipalId,
    ...
    if (this.#pullAttentions().length > 0) return null;
    if (!(await this.#discoverScopes(generation))) return null;
    if (!this.#isCurrentUser(generation, userId)) return null;
    const known = await this.#readKnownCommands();
    if (!this.#isCurrentUser(generation, userId)) return null;
    const command = known.find((candidate) => candidate.commandId === commandId);
    if (!command) return this.#generatedRemoteId(locator, userId, generation);
    if (command.userId !== userId) return null;
    if (
      command.scopeId !== locator.scopeId ||
      command.sourceKey !== locator.sourceKey ||
      command.identity.kind !== 'generated' ||
      command.identity.localId !== locator.localId
    ) {
      throw new Error('Immediate generated command locator does not match the durable command.');
    }
    ...
    if (aggregate[0]?.commandId !== command.commandId) {
      throw new Error('Immediate generated command delivery requires the first aggregate intent.');
    }
    ...
    if (
      ...
      row.confirmedValues !== null ||
      command.identity.kind !== 'generated' ||
      command.baseRevision !== null ||
      command.replicaMutation === 'delete'
    ) {
      throw new Error('Immediate generated command delivery requires a new unconfirmed aggregate.');
    }
    const scope = { userId: command.userId, scopeId: command.scopeId };
    const scopeKey = this.#scopeKey(scope);
    if (!this.#knownScopes.has(scopeKey)) return null;
    const dirtyScopes = new Map<string, OfflineScope>();
    await this.#sendAggregate([command], generation, dirtyScopes, new Set([scopeKey]));
    if (!this.#isCurrentUser(generation, userId)) return null;
  ...
  async #generatedCommandOutcome(
    ...
    if (!this.#isCurrentUser(generation, userId)) return null;
    if (completed && ['blocked_auth', 'rejected', 'conflict'].includes(completed.state)) {
      throw new OfflineImmediateCommandRejectedError(completed.state as 'blocked_auth' | 'rejected' | 'conflict', completed.lastErrorCode);
    }
    return completed
      ? (this.#commandReconciliationRemoteId(completed) ?? this.#generatedRemoteId(locator, userId, generation))
      : this.#generatedRemoteId(locator, userId, generation);
  }

  async #generatedRemoteId(
    locator: OfflineGeneratedCommandLocator,
    userId: OfflinePrincipalId,
    generation: number,
  ): Promise<OfflineGeneratedRemoteId | null> {
    if (!this.#isCurrentUser(generation, userId)) return null;
    const row = await this.#repository.getReplicaRow({ userId, scopeId: locator.scopeId }, locator.sourceKey, {
      kind: 'generated',
      localId: locator.localId,
    });
    return this.#isCurrentUser(generation, userId) && row?.identity.kind === 'generated' ? row.identity.remoteId : null;
  }

  #commandReconciliationRemoteId(command: OfflineCommand): OfflineGeneratedRemoteId | null {
    const identity = command.reconciliationIdentity;
    return identity && 'remoteId' in identity ? (identity.remoteId ?? null) : null;
  }
  ...
  async #sendAggregate(
    ...
    for (const command of commands) {
      if (!(await this.#sendCommandOnce(commands, command, generation, dirtyScopes, pulledScopeKeys))) break;
    }
  }

  #sendCommandOnce(
    aggregateCommands: OfflineCommand[],
    command: OfflineCommand,
    generation: number,
    dirtyScopes: Map<string, OfflineScope>,
    pulledScopeKeys: ReadonlySet<string>,
  ): Promise<boolean> {
    const transitionKey = this.#commandSendKey(command, generation);
    const active = this.#commandSendTransitions.get(transitionKey);
    if (active) return active;
    const transition = this.#sendCommand(aggregateCommands, command, generation, dirtyScopes, pulledScopeKeys).finally(() => {
      if (this.#commandSendTransitions.get(transitionKey) === transition) {
        this.#commandSendTransitions.delete(transitionKey);
      }
    });
    this.#commandSendTransitions.set(transitionKey, transition);
    return transition;
  }

  #commandSendKey(command: OfflineCommand, generation: number): string {
    return JSON.stringify([generation, canonicalOfflinePrincipalId(command.userId), command.commandId]);
  }
  ...
  async #sendCommand(
    ...
    if (command.state === 'awaiting_pull') return true;
    if (command.state === 'retry_wait' && (command.retryAt ?? 0) > Date.now()) return false;
    if (!['pending', 'retry_wait'].includes(command.state)) return false;
    ...
    if (!pulledScopeKeys.has(this.#scopeKey({ userId: command.userId, scopeId: command.scopeId }))) return false;
    let sending = await this.#claimSendingCommand(command, generation);
    if (!sending || !this.#isCurrent(generation)) return false;
    await this.#refreshState(generation);
    if (!this.#isCurrent(generation)) return false;
    ...
    if (!transportCommand) return false;
    ...
    if (!this.#isCurrent(generation)) return false;
    const completeCommand = async (): Promise<void> => this.#completeCommandWithRetry(aggregateCommands, sending, result, generation);
  ...
  #isCurrentUser(generation: number, userId: OfflinePrincipalId): boolean {
    return this.#isCurrent(generation) && this.#activeUserId === userId;
  }
